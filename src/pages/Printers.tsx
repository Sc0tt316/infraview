...
const Printers: React.FC = () => {
  ...
  const addPrinterMutation = useMutation({
    mutationFn: (printerData: Omit<PrinterData, "id" | "jobCount" | "lastActive">) => printerService.addPrinter(printerData),
    onSuccess: () => {
  ...
  useEffect(() => {
    if (printers) {
      ...
      if (searchTerm) {
        ...
        filtered = filtered.filter(printer => printer.name.toLowerCase().includes(lowerSearchTerm) || printer.model.toLowerCase().includes(lowerSearchTerm) || printer.location.toLowerCase().includes(lowerSearchTerm));
      }
      if (statusFilter !== 'all') {
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {canAddPrinter && <Button onClick={() => setShowAddPrinterModal(true)}>
      ...
      <PrinterFilters searchTerm={searchTerm} statusFilter={statusFilter} onSearchChange={handleSearchChange} onStatusFilterChange={handleStatusFilterChange} />
      
      {!isAdmin && <Alert className="bg-blue-50 border-blue-200 text-blue-800">
      ...
      <Dialog open={showAddPrinterModal} onOpenChange={setShowAddPrinterModal}>
        <DialogContent className="sm:max-w-[600px]">
          ...
          <AddPrinterForm form={form} onSubmit={handleAddPrinter} onCancel={() => setShowAddPrinterModal(false)} formData={formData} handleInputChange={handleInputChange} setFormData={setFormData} />
        </DialogContent>
      </Dialog>

      {selectedPrinter && <Dialog open={showEditPrinterModal} onOpenChange={handleCloseEditModal}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
            ...
            <EditPrinterForm form={form} onSubmit={handleUpdatePrinter} onCancel={handleCloseEditModal} formData={formData} handleInputChange={handleInputChange} setFormData={setFormData} />
          </DialogContent>
        </Dialog>}

      {selectedPrinter && <Dialog open={showDeleteConfirmation} onOpenChange={setShowDeleteConfirmation}>
          <DeletePrinterConfirmation printer={selectedPrinter} onDelete={handleDeletePrinter} onCancel={() => setShowDeleteConfirmation(false)} />
        </Dialog>}
