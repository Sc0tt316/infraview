...
const Servers = () => {
  ...
  const filteredServers = allServers.filter(server => {
    ...
    if (searchQuery) {
      ...
      return server.name.toLowerCase().includes(query) || server.hostname.toLowerCase().includes(query) || server.serverType.toLowerCase().includes(query) || server.location.toLowerCase().includes(query) || server.department && server.department.toLowerCase().includes(query);
    }
    return true;
  });
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <ServersHeader isLoading={isLoading} isRefreshing={isRefreshing} isAdmin={false} onRefresh={handleRefresh} onAddServer={() => {}} />
          {isAdmin && <AddServerDialog onAddServer={handleAddServer} />}
        </div>
      </div>

      <ServersContent isLoading={isLoading} filteredServers={filteredServers} searchQuery={searchQuery} selectedDepartment={departmentFilter} selectedStatus={statusFilter} isAdmin={isAdmin || false} onServerClick={handleServerClick} setSearchQuery={setSearchQuery} departmentFilter={departmentFilter} setDepartmentFilter={setDepartmentFilter} statusFilter={statusFilter} setStatusFilter={setStatusFilter} onRemoveServer={handleRemoveServer} onRefreshServer={handleRefreshServer} />
    </div>;
};
