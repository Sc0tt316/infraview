...
export interface AnalyticsData {
  ...
  printVolume: { // Add this
    date: string;
    volume: number;
  }[];
  alertHistory: { // Add this
    month: string;
    alerts: number;
  }[];
}
...
class AnalyticsService {
  ...
  async getPrintVolumeByDateRange(dateRange: { from: Date; to: Date }): Promise<PrintVolumeData[]> {
    // Simulate API request
    await new Promise(resolve => setTimeout(resolve, 600));
  ...
  async getActivityLogs(): Promise<ActivityLogData[]> {
    ...
    return Array.from({ length: 50 }, (_, i) => {
      ...
      const printer = hasPrinter ? printers[Math.floor(Math.random() * printers.length)] : undefined;
      ...
      return {
        ...
        details: action === 'printed document' 
          ? `Document: ${['Report', 'Invoice', 'Memo', 'Letter', 'Presentation'][Math.floor(Math.random() * 5)]}-${Math.floor(Math.random() * 1000)}.pdf` 
          : undefined,
      };
  ...
  async getPrinterActivity(printerId: string): Promise<ActivityLogData[]> {
    ...
    return activities.filter(activity => activity.printer?.includes('HP') || activity.printer?.includes('Canon')).slice(0, 10);
  }
